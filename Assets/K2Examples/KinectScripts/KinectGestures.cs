...
//		LeftHandCursor,
		...
		//Added by Marck
		SwipeLeftWithLeftHand,
		SwipeRightWithLeftHand,
		SwipeLeftWithRightHand,
		SwipeRightWithRightHand,
...
//				break;
			...
			case Gestures.SwipeRight:
				...
				//MARCK ADDED THIS 
			case Gestures.SwipeLeftWithLeftHand:
				float offsetToLeft = 0.1f;
				switch (gestureData.state)
				{
					case 0:  // gesture detection - phase 1

						//MARCK ADDED NEW STATE FOR RIGHT HIDE SWIPING TO THE LEFT : GESTURE DETECTION
						if (jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
						   jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
				   			jointsPos[leftHandIndex].x >= gestureLeft /**&& jointsPos[rightHandIndex].x > gestureLeft*/)
						{
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
							gestureData.progress = 0.1f;
						}
					...
					case 1:  // gesture phase 2 = complete
						...
						//MARCK ADDED NEW STATE FOR RIGHT HIDE SWIPING TO THE LEFT : GESTURE CANCELLATION
						if ((timestamp - gestureData.timestamp) <= 1.0f)
						{
							bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
									jointsPos[leftHandIndex].x <= gestureLeft - offsetToLeft;
							...
							{
								float gestureSize = gestureLeft - gestureRight;
								gestureData.progress = gestureSize > 0.01f ? (gestureLeft - jointsPos[rightHandIndex].x) / gestureSize : 0f;
							}
			...
			case Gestures.SwipeRightWithLeftHand:
				float offsetToRight = 0.1f;
				switch (gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if (jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
					...
					case 1:  // gesture phase 2 = complete
						if ((timestamp - gestureData.timestamp) <= 1.0f)
						{
							bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
									jointsPos[leftHandIndex].x >= gestureRight + offsetToRight;
			...
			case Gestures.SwipeRightWithRightHand:
				float offToRight = 0.1f;
				switch (gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if (jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
						   jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
				   			jointsPos[rightHandIndex].x <= gestureRight /**&& jointsPos[leftHandIndex].x < gestureRight*/)
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
							gestureData.progress = 0.1f;
						}
					...
					case 1:  // gesture phase 2 = complete
						if ((timestamp - gestureData.timestamp) <= 1.0f)
						{
							bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
									jointsPos[rightHandIndex].x >= gestureRight + offToRight;
							...
							{
								float gestureSize = gestureLeft - gestureRight;
								gestureData.progress = gestureSize > 0.01f ? (jointsPos[rightHandIndex].x - gestureRight) / gestureSize : 0f;
							}
						}
						else
						{
			...
			case Gestures.SwipeLeftWithRightHand:
				switch (gestureData.state)
				{
					case 0:  // gesture detection - phase 1
