...
public class InteractionManager : MonoBehaviour 
{
	...
	[Tooltip("Lets you see the other cursors beside the Primary Hand Cursor.")] //ADDED BY MARCK
	public bool IsDualHandCursor = true;
	...
	public Image guiHandCursorAlt;

	[Tooltip("Colors used to identify the primary cursor from the alternate cursor.")] //ADDED BY MARCK
	public Color PrimaryCursorColor = new Color(255,255,255, 255);
	public Color AlternateCursorColor = new Color(255, 255, 255, 255);
	...
	//ADDED BY MARCK
	public bool isRightHandSelectedAsPrimary;
	public bool isLeftHandSelectedAsPrimary;
	...
	public void SetLeftHandPrimary()
    {
		isLeftHandPrimary = true;
		isRightHandPrimary = false;
    }
	
	public void SetRightHandPrimary()
    {
	...
	void Update () 
	{
		...
		if(kinectManager && kinectManager.IsInitialized())
		{
			...
			if(playerUserID != 0)
			{
				...
				else if(isleftIboxValid && !leftHandInteraction && //bLeftHandPrimaryNow &&
				   kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
				{
					if (IsDualHandCursor) //ADDED BY MARCK
					{
						leftHandPos = kinectManager.GetJointPosition(playerUserID, (int)KinectInterop.JointType.HandLeft);
						leftHandScreenPos.z = Mathf.Clamp01((leftIboxLeftBotBack.z - leftHandPos.z) / (leftIboxLeftBotBack.z - leftIboxRightTopFront.z));
				...
				else if (isRightIboxValid && !rightHandInteraction && //bRightHandPrimaryNow &&
				   kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandRight) != KinectInterop.TrackingState.NotTracked)
				{
					if (IsDualHandCursor) //ADDED BY MARCK
					{
...
//				}
		...
		if(guiHandCursor)
		{
			...
			if((cursorTexture != null)) //&& (isLeftHandPrimary || isRightHandPrimary))
			{
				...
				else 
				{
					...
					//ADDED BY MARCK
					//This rotates the hand curson icon accordingly
					if (isLeftHandPrimary)
					{ 
						guiHandCursor.rectTransform.rotation = new Quaternion(0, -180, 0, 0);
					}
					else
					{
						guiHandCursor.rectTransform.rotation = new Quaternion(0, 0, 0, 0);
					}


					//ADDED BY MARCK
					//This sets a cursor for the non-primary hand
					if (IsDualHandCursor && guiHandCursorAlt)
					{
						/*
						if (!isLeftHandPrimary)
						{
							guiHandCursorAlt.rectTransform.anchoredPosition = new Vector2(leftHandScreenPos.x * rectCanvas.width, leftHandScreenPos.y * rectCanvas.height);
							guiHandCursorAlt.rectTransform.rotation = new Quaternion(0, -180, 0, 0);
						}
						else if(!isRightHandPrimary)
						{
							guiHandCursorAlt.rectTransform.anchoredPosition = new Vector2(rightHandScreenPos.x * rectCanvas.width, rightHandScreenPos.y * rectCanvas.height);
							guiHandCursorAlt.rectTransform.rotation = new Quaternion(0, 0, 0, 0);
						}
						*/
						if (isRightHandSelectedAsPrimary)
						{
							guiHandCursorAlt.rectTransform.anchoredPosition = new Vector2(leftHandScreenPos.x * rectCanvas.width, leftHandScreenPos.y * rectCanvas.height);
							guiHandCursorAlt.rectTransform.rotation = new Quaternion(0, -180, 0, 0);
						}
						else if (isLeftHandSelectedAsPrimary) 
						{
							guiHandCursorAlt.rectTransform.anchoredPosition = new Vector2(rightHandScreenPos.x * rectCanvas.width, rightHandScreenPos.y * rectCanvas.height);
							guiHandCursorAlt.rectTransform.rotation = new Quaternion(0, 0, 0, 0);
						}
                ...
                if (IsDualHandCursor && guiHandCursorAlt)
                {
					guiHandCursor.color = PrimaryCursorColor;
					guiHandCursorAlt.color = AlternateCursorColor;
				}
	...
	public void LockAlternateCursor(Vector3 cursorPosition, Transform cursor)
    {
